...
vec startobeta(vec const& betastar){
  ...
  //Unrestricted Atts. 
  beta.subvec(2,3) = betastar.subvec(2,3); 
...
double llmnl_BC(vec const& betastar,double const& stated_budget,double error_sd, vec const& y, mat const& X, int const& pr){
  ...
  double lik = sum(xby - log(denom)) + log_normpdf(stated_budget,budget,error_sd);
  return(lik);    
}
...
mnlMetropOnceOut mnlMetropOnce_mod(vec const& y, mat const& X, vec const& oldbeta, 
                                   ...
                                   vec const& betabar, mat const& rootpi, int const& pr,double const& stated_budget,double const& error_sd){ 
  ...
  double cll = llmnl_BC(betac,stated_budget,error_sd,y,X,pr);
...
List rhierMnlRwMixture_rcpp_loop_mod(List const& lgtdata, int nvar_c, mat const& Z, vec const& deltabar, mat const& Ad, mat const& mustarbarc, mat const& Amu,
                                  ...
                                  vec ind_stay_temp, mat ind_stay, int ncomp, int const& pr,vec const& stated_budget){
...
// Modified by Max Pachali & Thomas Otter to include sign & order constraints (log-normal prior)
  ...
  // sd of measurement error in population
  double s_error;
  int nu_error = 3; //see Rossi 2005
  double ssq_error = 0.1;
  double error_sd_temp;
  vec budget_inf(nlgt);
  vec error_sd(R/keep);
  ...
  for (int rep = 0; rep<R; rep++){
    ...
    //now draw error | beta
    budget_inf = exp(oldbetas(span::all,0));  
    vec diff = stated_budget-budget_inf;
    vec diff_sq = pow(diff,2);
    s_error = sum(diff_sq);
    error_sd_temp = sqrt(((nu_error*ssq_error+s_error) / rchisq(1,nu_error+nlgt)[0])); //rchisq returns a vectorized object, so using [0] allows for the conversion to double
    ...
    //loop over all LGT equations drawing beta_i | ind[i],z[i,],mu[ind[i]],rooti[ind[i]]
      for(int lgt = 0; lgt<nlgt; lgt++){
        ...
        if (rep == 0) oldll[lgt] = llmnl_BC(vectorise(oldbetas(lgt,span::all)),stated_budget[lgt],error_sd_temp,lgtdata_vector[lgt].y,lgtdata_vector[lgt].X, pr);
        ...
        metropout_struct = mnlMetropOnce_mod(lgtdata_vector[lgt].y,lgtdata_vector[lgt].X,vectorise(oldbetas(lgt,span::all)),
                                         oldll[lgt],s,incroot,betabar,rootpi,pr,stated_budget[lgt],error_sd_temp);
    ...
    if((rep+1)%keep==0){
      ...
      error_sd[mkeep-1] = error_sd_temp;
...
// [[Rcpp::export]]
double lndM(double const& x, double const& mu, double const& rooti){
  ...
  return((-(1/2.0)*log(2*M_PI) -.5*(z*z) + log(rooti)));
}
